...
namespace SISPE_MIGRACION.formularios.Seguros
{
    public partial class frmseguro : Form
    {
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            ...
            string conjunto = fecha.Substring(2, 2) + fecha.Substring(5, 2);


            //Creación de la tabla temporal del estado de cuenta
            string query = "CREATE TEMP TABLE primer AS SELECT	folio,	f_emischeq,	rfc,	nombre_em,	secretaria, " +
                " f_primdesc,	proyecto,	importe,	ubic_pagare,	imp_unit , concat( substr( cast(fallecio as VARCHAR) , 3,2) , substr( cast(fallecio as VARCHAR) , 6,2)  ) as murio FROM " +
            ...
            query += "update primer set murio = 0 where murio = '';";
            query += $"create temp table edo as  select* from primer where (murio::numeric >= {conjunto} or murio = '0'); ";
            ...
            globales.showModal(new frmlistaseguro(resultado, rdQuiro.Checked, string.Format("{0:d}", dFecha.Value)));
            this.Cursor = Cursors.Default;
        }

        private void frmsaldos_FormClosing(object sender, FormClosingEventArgs e)
        {
            DialogResult dialogo = globales.MessageBoxQuestion("¿Desea cerrar el módulo?", this);
